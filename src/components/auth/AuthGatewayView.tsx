...
export function AuthGatewayView({ onPhoneSelect, isLoading }: AuthGatewayViewProps) {
  const supabase = createClient();

  const handleGoogleAuth = async () => {
    const baseUrl = typeof window !== 'undefined' ? window.location.origin : '';
    // Redirect to a specialized callback that knows how to handle the personalization step
    const redirectTo = `${baseUrl}/auth/callback?next=/signup&flow=high-velocity`;
    ...
    >
      ...
      <div className="flex flex-col gap-4">
        ...
        <div className="flex items-center gap-4 py-2">
          ...
          <span className="text-[10px] font-black uppercase tracking-widest text-on-surface-variant/40">or use phone</span>
          <div className="h-px bg-outline-variant flex-1 opacity-50" />
        </div>
        ...
        <Button
          ...
          variant="ghost"
          className="h-16 rounded-[2rem] text-lg font-black text-on-surface hover:bg-primary/5 transition-all flex items-center justify-center gap-3 active:scale-95"
        >
